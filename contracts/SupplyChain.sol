...
contract SupplyChain {
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].seller != address(0), "This item isn't up for sale");
    _;
  }
  modifier sold(uint _sku) {
    require(items[_sku].buyer != address(0), "This item has been sold");
    _;
  } 
  modifier shipped(uint _sku) {
    require(items[_sku].state == State.Shipped, "This item has been shipped");
    _;
  }
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, "This item has been recieved");
    _;
  }
